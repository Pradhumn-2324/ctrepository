    ...
    return (
        <div className='w-[100vw] bg-[#000000] h-auto text-white py-[60px]'>
            <div className='w-[85%] mx-[110px]'>
                <h1 className='text-3xl font-bold'>Mobile Apps We Develop</h1>
                <p className='font-semibold text-[14px] mt-[20px]'>Mobile app development services are aimed at building iOS & Android applications that effectively complement or substitute web solutions. Having delivered 350+ mobile app development projects so far, ScienceSoft ensures app success by delivering striking UI, secure app code, and resilient back ends.</p>
            </div>
            <div className='w-[85%] flex flex-wrap justify-between mx-[110px]'>
                {
                    areaofExpertiseData.map((item, index) => {
                        return (
                            <div key={index} className='w-[280px] h-[220px] mt-[30px] bg-[black] opacity-[0.6] relative' style={{ backgroundImage: `url(${item.expertiseImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }} onMouseEnter={() => setHoveredIndex(index)} onMouseLeave={() => setHoveredIndex(null)}>
                                <button className='border border-[white] px-[20px] w-[150px] h-[40px] absolute bottom-[-15px] left-[-5px] text-center bg-cyan-500 cursor-pointer text-[14px]' style={{ opacity: hoveredIndex === index ? 1 : 0, transition: 'opacity .4s ease-in-out' }}>Explore More &rarr;</button>
                                <button className='border border-[white] px-[10px] absolute bottom-[-15px] left-[-5px] bg-[#474747] w-[220px] h-[40px] text-center text-[white] text-[14px]' style={{ opacity: hoveredIndex === index ? 0 : 1, transition: 'opacity .4s ease-in-out' }}>{item.expertiseIn}</button>
                            </div>
                        )
                    })
                }
